...
async def read_users_me(current_user: User = Depends(auth_service.get_current_user)):
    """
        Retrieve the authenticated user's own information.

        Parameters:
        - current_user (User): The authenticated User object.

        Returns:
        - UserDb: The UserDb object representing the authenticated user.
        """
    return current_user
...
async def update_avatar_user(file: UploadFile = File(), current_user: User = Depends(auth_service.get_current_user),
    ...
    """
       ...
       Returns:
       - UserDb: The UserDb object representing the updated user.
       """
    cloudinary.config(
